...
const CLAIMS_STATUS_CONFIG: Record<string, { label: string; icon: any; color: string; bg: string }> = {
  pending: { label: "Claims pending", icon: Clock, color: "text-gray-500 dark:text-slate-400", bg: "bg-gray-50 border-gray-200 dark:border-slate-700" },
...
function ArticleCard({ article }: ArticleCardProps) {
  ...
  return (
    <Link
      ...
      className={clsx(
        "block bg-white dark:bg-slate-800 rounded-xl shadow-sm overflow-hidden group",
      ...
      <div className="p-6">
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1 flex items-center gap-2">
            <span className="text-sm font-medium text-gray-900 dark:text-slate-100">{article.source_name}</span>
            {article.content_category && (
              <span className="text-[10px] text-gray-400 dark:text-slate-500 px-1.5 py-0.5 bg-gray-50 dark:bg-slate-800 rounded border border-gray-100 dark:border-slate-800">
        ...
        <h3 className="text-xl font-bold text-gray-900 dark:text-slate-100 mb-3 group-hover:text-clilens-primary dark:group-hover:text-teal-400 transition-colors line-clamp-2">
        ...
        {(article.enriched_excerpt || article.excerpt) && (
          <div className="text-gray-600 dark:text-slate-400 text-sm mb-4 line-clamp-6">
        ...
        {article.climate_context_summary && (
          <div className="flex items-start gap-2 mb-4 px-3 py-2 bg-teal-50 dark:bg-teal-900/30 border border-teal-200 dark:border-teal-800 rounded-lg">
            <Thermometer className="h-4 w-4 text-teal-600 dark:text-teal-400 mt-0.5 flex-shrink-0" />
            <p className="text-xs text-teal-700 dark:text-teal-300 line-clamp-2">{article.climate_context_summary}</p>
          </div>
        )}
        ...
        {Array.isArray(article.tags) && article.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {article.tags
              ...
              .slice(0, 4)
              .map((tag) => (
                <span key={tag} className="px-2 py-0.5 bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-400 text-xs rounded-full border border-gray-200 dark:border-slate-700">
                  {formatTagLabel(tag)}
                </span>
              ))}
          </div>
        ...
        {article.claims_by_category && Object.keys(article.claims_by_category).length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {Object.entries(article.claims_by_category).map(([cat, count]) => {
              ...
              return (
                <span key={cat} className="inline-flex items-center space-x-0.5 px-2 py-0.5 rounded-full text-[10px] font-medium bg-gray-100 text-gray-500 dark:text-slate-400 border border-gray-200 dark:border-slate-700">
          ...
          return (
            <div className={clsx("flex items-center space-x-1.5 px-2.5 py-1.5 rounded-md border text-xs mb-4", cfg.bg)}>
              ...
              {article.claims_error_message && (
                <span className="text-gray-400 dark:text-slate-500 ml-1 truncate max-w-[180px]">— {article.claims_error_message}</span>
              )}
            </div>
          );
        })()}
        ...
        <div className="pt-4 border-t border-gray-100 dark:border-slate-800">
          <div className="flex items-center justify-between">
            ...
            {article.claim_count > 0 && (
              <div className="flex items-center space-x-2">
                <div className="flex-1 bg-gray-200 dark:bg-slate-700 rounded-full h-2 w-32">
                  <div
                    className="bg-gradient-to-r from-clilens-primary to-clilens-teal-600 h-2 rounded-full transition-all"
                    style={{ width: `${verificationRate}%` }}
                  />
                </div>
                <span className="text-sm font-medium text-clilens-teal-600 dark:text-teal-400">{verificationRate}%</span>
              </div>
            )}
          </div>
        </div>

  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900">
      <div className="bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex items-center justify-between">
          ...
          <nav className="text-sm text-gray-500 dark:text-slate-400 flex gap-4">
            <Link href="/about" className="hover:text-gray-800 dark:hover:text-slate-200 dark:text-slate-200">About</Link>
            <Link href="/methodology" className="text-gray-900 dark:text-slate-100 font-medium">Methodology</Link>
            <Link href="/sources" className="hover:text-gray-800 dark:hover:text-slate-200 dark:text-slate-200">Sources</Link>
          </nav>
        </div>
      </div>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
        <header>
          <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100 mb-3">Our Methodology</h1>
          ...
          {bundle?.git_revision && (
            <p className="mt-3 text-xs text-gray-500 dark:text-slate-400 flex items-center gap-1.5">
              <GitCommit className="w-3.5 h-3.5" />
              Live snapshot pinned to commit{" "}
              <code className="bg-gray-100 dark:bg-slate-700 px-1.5 py-0.5 rounded font-mono">{bundle.git_revision}</code>
            </p>
          )}
        </header>

        {/* Section: Self-audit — the audit gap ---------------------------------- */}
        <section className="bg-gradient-to-r from-teal-50 to-emerald-50 dark:from-slate-800 dark:to-slate-800 rounded-xl shadow-sm border border-teal-200 dark:border-teal-800 p-6 space-y-5">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100 flex items-center gap-2.5">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-white/80 dark:bg-slate-800/80 rounded-lg border border-teal-100 p-4">
              <div className="text-xs uppercase tracking-wider text-teal-700 dark:text-teal-400 font-semibold mb-2">
          ...
          {selfAudit?.axes && selfAudit.axes.length > 0 && (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 text-xs mt-4">
              {selfAudit.axes.map((axis) => (
                <div
                  key={axis.axis}
                  className="bg-white/80 dark:bg-slate-800/80 rounded border border-gray-200 dark:border-slate-700 p-2 flex justify-between items-start"
                >
                  <div>
                    <div className="font-medium text-gray-900 dark:text-slate-100 capitalize">
                      {axis.axis.replace(/_/g, " ")}
                    </div>
                    <div className="text-gray-500 dark:text-slate-400">{axis.detail}</div>
                  </div>
          ...
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-slate-100 text-sm mb-2">What the audit found</h3>
            <div className="grid sm:grid-cols-2 gap-2 text-xs">
              {[
                ...
                <div key={row.axis} className="bg-white/80 dark:bg-slate-800/80 rounded border border-gray-200 dark:border-slate-700 p-3">
                  <div className="font-medium text-gray-900 dark:text-slate-100">{row.axis}</div>
                  <div className="mt-1 flex gap-2 items-baseline">
                    <span className="text-teal-700 dark:text-teal-400">Claimed: {row.before}</span>
                    <span className="text-gray-300 dark:text-slate-600">→</span>
                    <span className="text-amber-700 dark:text-amber-400 font-medium">Audited: {row.after}</span>
                  </div>
                  <div className="text-gray-500 dark:text-slate-400 mt-1">{row.fix}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        >
          ...
          <div className="grid sm:grid-cols-2 gap-3 text-sm">
            {[
              ...
              <div
                key={card.area}
                className="bg-gradient-to-br from-gray-50 to-white dark:from-slate-800 dark:to-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-3"
              >
                <h4 className="text-xs uppercase tracking-wider text-emerald-700 dark:text-emerald-400 font-semibold mb-2">
                  {card.area}
                </h4>
          ...
          <p className="text-xs text-gray-500 dark:text-slate-400 italic border-t border-gray-200 dark:border-slate-700 pt-3">
        ...
        <section className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 space-y-5">
          ...
          <div className="grid md:grid-cols-2 gap-3 pt-3">
            <div className="bg-green-50 dark:bg-emerald-900/30 border border-green-200 dark:border-emerald-800 rounded p-3 text-sm">
              <div className="flex items-center gap-2 font-semibold text-green-900 dark:text-emerald-200">
                <CheckCircle className="w-4 h-4" /> What we surface
              </div>
              <ul className="list-disc list-inside text-xs text-green-800 dark:text-emerald-300 mt-1 space-y-0.5">
            ...
            <div className="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 rounded p-3 text-sm">
              <div className="flex items-center gap-2 font-semibold text-yellow-900 dark:text-yellow-200">
                <AlertTriangle className="w-4 h-4" /> Known limits
              </div>
              <ul className="list-disc list-inside text-xs text-yellow-800 dark:text-yellow-300 mt-1 space-y-0.5">
        ...
        <Section
          icon={<Route className="w-5 h-5 text-teal-600 dark:text-teal-400" />}
        ...
        <section className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 space-y-3">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100 flex items-center gap-2">
            Verdict labels
            <AskAboutButton
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            <VerdictRow icon={<CheckCircle className="w-4 h-4 text-green-600" />}
              label="Verified"
              tone="bg-green-50 dark:bg-emerald-900/30 border-green-200 dark:border-emerald-800 text-green-900 dark:text-emerald-200"
            ...
            <VerdictRow icon={<AlertTriangle className="w-4 h-4 text-yellow-600" />}
              ...
              tone="bg-yellow-50 dark:bg-yellow-900/30 border-yellow-200 dark:border-yellow-800 text-yellow-900 dark:text-yellow-200"
            ...
            <VerdictRow icon={<XCircle className="w-4 h-4 text-red-600" />}
              ...
              tone="bg-red-50 dark:bg-red-900/30 border-red-200 dark:border-red-800 text-red-900 dark:text-red-200"
              description="Scientific consensus contradicts the claim." />
            <VerdictRow icon={<HelpCircle className="w-4 h-4 text-gray-600 dark:text-slate-400" />}
              label="Unverified"
              tone="bg-gray-50 dark:bg-slate-800 border-gray-200 dark:border-slate-700 text-gray-900 dark:text-slate-100"
        ...
        {/* Section: Feedback ---------------------------------------------- */}
        <section className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 space-y-3">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Feedback & corrections</h2>
          <p className="text-sm text-gray-700 dark:text-slate-300">
          ...
          <p className="text-xs text-gray-500 dark:text-slate-400 pt-1">
            ...
            <code className="bg-gray-100 dark:bg-slate-700 px-1 py-0.5 rounded font-mono">
        ...
        >
          <div id="corporate-verification" className="space-y-3 text-sm text-gray-700 dark:text-slate-300">
            <p>
              Each claim is routed through a rule set pinned by{" "}
              <code className="bg-gray-100 dark:bg-slate-700 px-1 py-0.5 rounded font-mono text-xs">
        ...
        <footer className="text-xs text-gray-500 dark:text-slate-400 pt-6 border-t border-gray-200 dark:border-slate-700">
          Methodology snapshot generated live from{" "}
          <code className="bg-gray-100 dark:bg-slate-700 px-1.5 py-0.5 rounded font-mono">GET /api/methodology</code>{" "}
...
function Section({
  ...
  return (
    <section className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 space-y-4">
      <header>
        <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100 flex items-center gap-2.5">
          {icon}
          {title}
        </h2>
        <p className="text-sm text-gray-600 dark:text-slate-400 mt-1 max-w-3xl">{intro}</p>
      </header>
      {children}
    </section>
  );
...
function ProcessStep({ n, title, body }: { n: number; title: string; body: string }) {
  return (
    <li className="flex gap-3">
      <span className="flex-shrink-0 w-7 h-7 rounded-full bg-teal-100 dark:bg-teal-900/40 text-teal-700 dark:text-teal-400 font-semibold text-sm flex items-center justify-center">
        {n}
      </span>
      <div>
        <h3 className="font-semibold text-gray-900 dark:text-slate-100 text-sm">{title}</h3>
        <p className="text-sm text-gray-700 dark:text-slate-300">{body}</p>
      </div>
    </li>
  );
}
...
function Unavailable({ reason }: { reason: string }) {
  return (
    <div className="text-sm text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded px-3 py-2">
      Unavailable — {reason}.
    </div>
  );
}
  ...
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full text-sm">
        <thead>
          <tr className="text-left text-xs uppercase tracking-wider text-gray-500 dark:text-slate-400 border-b border-gray-200 dark:border-slate-700">
        ...
        <tbody>
          {entries.map(([name, meta]) => (
            <tr key={name} className="border-b border-gray-100 dark:border-slate-800 align-top">
              <td className="py-2 pr-3 font-mono text-gray-900 dark:text-slate-100">{name}</td>
              <td className="py-2 pr-3 font-mono text-teal-700 dark:text-teal-400">{meta.version}</td>
              <td className="py-2 pr-3 font-mono text-xs text-gray-500 dark:text-slate-400" title={meta.fingerprint}>
                <span className="inline-flex items-center gap-1">
                  <Fingerprint className="w-3 h-3" />
                  {meta.fingerprint}
                </span>
              </td>
              <td className="py-2 pr-3 text-gray-700 dark:text-slate-300 max-w-md">{meta.description || "—"}</td>
              <td className="py-2 pr-3 text-gray-600 dark:text-slate-400 max-w-md text-xs">{meta.rationale || "—"}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function FormulaBlock({ formula }: { formula: MethodologyBundle["sustainability_formula"] }) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-700 dark:text-slate-300">{formula.scoring_summary}</p>
      <div className="text-xs text-gray-500 dark:text-slate-400">
        Methodology version:{" "}
        <code className="bg-gray-100 dark:bg-slate-700 px-1 py-0.5 rounded font-mono">{formula.methodology_version}</code>
        {formula.methodology_url && (
          <>
            {" · "}
            <a className="text-teal-700 dark:text-teal-400 hover:underline" href={formula.methodology_url} target="_blank" rel="noreferrer">
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {formula.components.map((c) => (
              <tr key={c.indicator_id} className="border-b border-gray-100 dark:border-slate-800 align-top">
                <td className="py-2 pr-3 font-mono text-gray-900 dark:text-slate-100">{c.indicator_id}</td>
                <td className="py-2 pr-3 font-medium text-teal-700 dark:text-teal-400">{Math.round(c.weight * 100)}%</td>
                <td className="py-2 pr-3 text-gray-700 dark:text-slate-300">{c.description}</td>
                <td className="py-2 pr-3 text-xs text-gray-600 dark:text-slate-400">
                  <code className="bg-gray-100 dark:bg-slate-700 px-1 py-0.5 rounded font-mono mr-1">{c.normalizer}</code>
                  {c.normalizer_doc}
                </td>
              </tr>
            ))}
      ...
      <div>
        <h4 className="text-xs uppercase tracking-wider text-gray-500 dark:text-slate-400 mb-1">Confidence bands</h4>
        <div className="flex flex-wrap gap-2 text-xs">
          {formula.confidence_band_table.map((row) => (
            <div key={row.indicators_used} className="bg-gray-100 dark:bg-slate-700 rounded px-2 py-1">
              <span className="text-gray-500 dark:text-slate-400">{row.indicators_used} indicators →</span>{" "}
              <span className="font-mono text-gray-900 dark:text-slate-100">±{row.band_plus_minus}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      {Object.entries(byCategory).map(([category, items]) => (
        <div key={category}>
          <h4 className="text-xs uppercase tracking-wider text-gray-500 dark:text-slate-400 mb-1">{category}</h4>
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead>
              ...
              <tbody>
                {items.map((i) => (
                  <tr key={i.indicator_id} className="border-b border-gray-100 dark:border-slate-800">
                    <td className="py-1.5 pr-3">
                      <div className="font-mono text-gray-900 dark:text-slate-100">{i.indicator_id}</div>
                      <div className="text-xs text-gray-500 dark:text-slate-400">{i.display_name}</div>
                    </td>
                    <td className="py-1.5 pr-3 text-gray-700 dark:text-slate-300">{i.unit}</td>
                    <td className="py-1.5 pr-3 text-xs">
                      {i.is_higher_better ? (
                        <span className="text-teal-700 dark:text-teal-400">higher is better</span>
                      ) : (
                    ...
                    <td className="py-1.5 pr-3">
                      {i.methodology_url ? (
                        <a className="text-teal-700 dark:text-teal-400 hover:underline text-xs" href={i.methodology_url} target="_blank" rel="noreferrer">
...
function CalibrationCard({ signal, data }: { signal: string; data: CalibrationResponse | null }) {
  ...
  if (!data.available) {
    return (
      <div className="border border-gray-200 dark:border-slate-700 rounded-lg p-4">
        <h3 className="font-semibold text-gray-900 dark:text-slate-100 mb-1">{signal}</h3>
        <p className="text-xs text-amber-700 dark:text-amber-400">Unavailable: {data.reason || "no data"}</p>
      </div>
    );
  }
  ...
  const fitBadge: Record<string, { label: string; cls: string }> = {
    stable: { label: "Stable fit", cls: "bg-green-100 text-green-800 dark:text-emerald-300" },
    preview: { label: "Preview fit", cls: "bg-amber-100 text-amber-800 dark:text-amber-300" },
    insufficient_data: { label: "Insufficient data", cls: "bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-slate-300" },
    no_labels: { label: "No labels", cls: "bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-slate-400" },
  };
  const badge = fitBadge[fitStatus];
  return (
    <div className="border border-gray-200 dark:border-slate-700 rounded-lg p-4 space-y-1">
      <div className="flex items-center justify-between gap-2">
        <h3 className="font-semibold text-gray-900 dark:text-slate-100">{signal}</h3>
        {badge && (
      ...
      <p className="text-xs text-gray-600 dark:text-slate-400">{m.note || ""}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="bg-gray-50 rounded p-3 text-sm">
        ...
        <div className="flex gap-6 text-gray-900 dark:text-slate-100">
...
function HalRateMiniTable({
  ...
  return (
    <div>
      <h4 className="text-xs uppercase tracking-wider text-gray-500 dark:text-slate-400 mb-1">{label}</h4>
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          <thead>
            <tr className="text-left text-xs text-gray-500 dark:text-slate-400 border-b border-gray-200 dark:border-slate-700">
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr key={i} className="border-b border-gray-100 dark:border-slate-800">
                <td className="py-1 pr-3 font-mono text-gray-900 dark:text-slate-100 text-xs">{String(row[keyField] ?? "unknown")}</td>
                <td className="py-1 pr-3 text-gray-700 dark:text-slate-300">{row.n}</td>
                <td className="py-1 pr-3 text-gray-700 dark:text-slate-300">{(row.mean_risk * 100).toFixed(1)}%</td>
                <td className="py-1 pr-3 text-gray-700 dark:text-slate-300">{(row.high_risk_rate * 100).toFixed(1)}%</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function DriftCard({
  ...
  if (data === null) {
    return <div className="border border-gray-200 dark:border-slate-700 rounded-lg p-4"><Loading /></div>;
  }
  const verdictTone: Record<string, string> = {
    ...
    insufficient_data: "bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-400 border-gray-300",
    stable: "bg-teal-50 dark:bg-teal-900/30 text-teal-800 dark:text-teal-300 border-teal-200 dark:border-teal-800",
    minor: "bg-yellow-50 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300 border-yellow-200 dark:border-yellow-800",
    notable: "bg-orange-50 text-orange-800 border-orange-200",
    significant: "bg-red-50 dark:bg-red-900/30 text-red-800 border-red-200 dark:border-red-800",
  };
  const tone = verdictTone[data.verdict] || "bg-gray-50 text-gray-800 dark:text-slate-200 border-gray-200 dark:border-slate-700";
  ...
  return (
    <div className="border border-gray-200 dark:border-slate-700 rounded-lg p-4 space-y-3">
      <header className="flex items-center justify-between">
        <h3 className="font-semibold text-gray-900 dark:text-slate-100">{label}</h3>
        <span className={`text-xs px-2 py-0.5 rounded border ${tone}`}>{verdictLabel}</span>
      </header>
      <div className="text-xs text-gray-500 dark:text-slate-400">
        KL divergence:{" "}
        <strong className="text-gray-900 dark:text-slate-100 font-mono">{data.kl_divergence.toFixed(3)}</strong>
        {" · "}{data.recent_count} recent / {data.baseline_count} baseline
      </div>
      {data.top_shifts && data.top_shifts.length > 0 && (
        <div>
          <h4 className="text-xs uppercase tracking-wider text-gray-500 dark:text-slate-400 mb-1">Top shifts</h4>
            ...
            {data.top_shifts.slice(0, 5).map((s, i) => {
              ...
              return (
                <li key={i} className="flex items-center justify-between">
                  <code className="text-gray-700 dark:text-slate-300 font-mono truncate max-w-[60%]">{name}</code>
                  <span className={delta >= 0 ? "text-teal-700 dark:text-teal-400" : "text-red-700"}>
      ...
      {data.notes && <p className="text-xs text-gray-500 dark:text-slate-400 italic">{data.notes}</p>}
    </div>
  );
}
  ...
  return (
    <li>
      <a href={href} target="_blank" rel="noreferrer"
         className="flex items-start gap-2 text-teal-700 dark:text-teal-400 hover:underline">

        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <CloudSun className="h-5 w-5 text-clilens-primary" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="mb-6">
          ...
          <select
            ...
            className="px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-clilens-primary focus:border-transparent w-64"
          >
            {COUNTRIES.map((c) => (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{forecast.date_range}</p>
                </div>
                <DiscrepancyBadge score={forecast.discrepancy_score} />
              </div>
              <p className="text-sm text-gray-600 mb-4">{forecast.consensus_summary}</p>

              {/* Confidence bar */}
              <div className="flex items-center gap-3">
                <span className="text-xs text-gray-500">Composite confidence:</span>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {forecast.sources.map((src) => (
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-sm font-bold text-gray-900">{src.source_name}</h3>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200">
                      ...
                      {forecast.sources.map((s) => (
                        <th key={s.source_name} className="text-right py-2 px-3 text-gray-500 font-medium">
                          {s.source_name}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    <tr className="border-b border-gray-100">
                      <td className="py-2 pr-4 text-gray-600">Temperature (&deg;C)</td>
                      {forecast.sources.map((s) => (
                        <td key={s.source_name} className="text-right py-2 px-3 font-medium text-gray-900">
                          {s.temperature_avg != null ? `${s.temperature_avg}` : "—"}
                        </td>
                      ))}
                    </tr>
                    <tr className="border-b border-gray-100">
                      <td className="py-2 pr-4 text-gray-600">Precipitation (mm)</td>
                      {forecast.sources.map((s) => (
                        <td key={s.source_name} className="text-right py-2 px-3 font-medium text-gray-900">
                    ...
                    <tr className="border-b border-gray-100">
                      <td className="py-2 pr-4 text-gray-600">Wind (m/s)</td>
                      {forecast.sources.map((s) => (
                        <td key={s.source_name} className="text-right py-2 px-3 font-medium text-gray-900">

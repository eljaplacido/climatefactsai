...
|---|---|---|---|
| F1 | **Off-topic ingestion** (bus-accident class) | BROKEN — no relevance gate | §3; `ingestion.py:376-388`, `rss_adapter.py:213-286,465-495`; wire `editorial_gate.py` |
...
| F2 | **Chat "Claim:" contrast** (Bug1.png) | `VERIFY-ON-LIVE` — code looks OK | `AgenticAssistant.tsx:545,632,637` use slate-800 bg + slate-200 text (paired). Reproduce on live with hard refresh; if fixed, close with note |
| F4 | **Article export failing** | `VERIFY-ON-LIVE` — code wired | `ArticleExportButtons.tsx:34-101`, `src/frontend/src/lib/api.ts:479-495` (`/api/export/article/{id}/{pdf,csv}`), rendered `articles/[id]/page.tsx:298`. If failing, it's backend/perms — test endpoints live |
...
| F7 | **KG as text rows + "cookies" entity + SDG copy** | BROKEN/PARTIAL | relationships render as ASCII rows `KnowledgeGraphMini.tsx:234-259`; related-coverage rows `:262-295`; **no graph viz lib** → add one (react-flow/vis-network/d3). No boilerplate filter in `entity_extraction_service.py` → "cookies"/consent leak; add stopword/boilerplate filter. SDG chips minimal `SDGChips.tsx:60-79` → add real copy |

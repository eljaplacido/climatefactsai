...
|---|---|---|---|
| F1 | **Off-topic ingestion** (bus-accident class) | WORKS (code, 2026-06-01) — ingest gate + display filter across ~40 sites (`8394b56`) + curated backfill (mig 057) + **LLM relevance classifier & `POST /api/admin/backfill/relevance-flag`** (`7c7b204`, review-hardened `553ae8e`) that flags the bus-class corpus-wide (validated: bus→OFF 0.00). **Operational step:** owner runs the backfill on prod (dry-run→review→apply) to flag existing rows; ingest gate handles new ones. | §3; `relevance_classifier.py`; `api/admin_backfill_routes.py` (relevance-flag); mig 056/057/059 |
...
| F8a | **Research "audit trail" = raw JSON** | DEFERRED (2026-06-01) — relabel shipped earlier; full readable report not built: **(1)** all local `url_analyses` rows are `failed` (no completed data to render against), **(2)** architectural split — the list (`GET /api/research/analyses`) reads `url_analyses` but `analyze_report` writes `saved_analyses`; reconcile first. Spec: add `GET /api/research/analyses/{id}` (return the url_analyses row) + `/research/analysis/[id]/page.tsx` rendering title/claims/fact_checks/credibility + SDGChips; relink `RecentResearchAnalyses.tsx`. Needs a completed analysis to verify. | `RecentResearchAnalyses.tsx`; `api/research_routes.py`; `research_report_service.py:113` |
...
|---|---|---|---|
| F3 | **Theme switching removed** | DECISION → DEFER (2026-06-01 recon) — measured **24% `dark:` coverage**, no ThemeProvider, gradient/custom-colour token gaps. A real build is **XL (5-7 days)** + high regression risk; removing the 469 dead tokens destroys the groundwork. Pinned-light stays until a dedicated ThemeProvider effort. Owner's build-vs-remove call. | `layout.tsx:45`, `GlobalNav.tsx`, `tailwind.config.js` |
...
| F5c | **Deep-search "weak evidence" default** | WORKS (2026-06-01, `e3c2e8c`) — prominent "Why this answer is low-confidence" banner above the answer: `confidence_envelope.reason` + internal/external counts + actionable next step keyed to the platform-only toggle. (F5d academic-corpus still open.) | `app/deep-search/page.tsx` |
...
| F9c | **PPP (Planet/People/Profit) lens** | WORKS (honest minimal, 2026-06-01, `e3c2e8c`) — Planet populated (Scope 1-3, SBTi, net-zero); People/Profit are explicit "not yet ingested" empty states naming the missing source (GRI/CSRD social, annual-report financials). Full version (PDF parsing) deferred. | `companies/[ticker]/page.tsx` |
...
| F11 | **Unrated sources** | IMPROVED (2026-06-01, `6ed2f9a`) — mig 058 tiers 41 more corpus sources with no prior match (16 T1 / 17 T2 / 8 T3: NASA, NOAA, CSIRO, Nature Climate Change, Copernicus, Clean Energy Wire, DeSmog, Reporterre, SCMP/The Hindu/NPR climate desks, met agencies…). Idempotent `ON CONFLICT (domain)`. Residual long-tail may remain → re-scan + extend. | mig 058; `source_profiles.py:303-366` |

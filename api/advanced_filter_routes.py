...
async def filter_articles(request: FilteredArticleRequest):
    ...
    conditions = ["1=1", "a.is_synthetic = FALSE", "a.is_off_topic = FALSE"]
...
async def explore_topics(country: Optional[str] = None, limit: int = Query(default=30, le=100)):
    ...
    try:
        rows = db.execute_query(f"""
            ...
            WHERE a.is_synthetic = FALSE AND a.is_off_topic = FALSE AND a.tags IS NOT NULL {country_filter}
...
async def explore_sources(country: Optional[str] = None, limit: int = Query(default=50, le=200)):
    ...
    db = get_postgres()
    conditions = ["a.is_synthetic = FALSE", "a.is_off_topic = FALSE"]
    params: Dict[str, Any] = {"limit": limit}
    if country:
...
async def get_article_trends(request: TimeSeriesRequest):
    ...
    conditions = [f"a.created_at >= NOW() - interval '{request.days} days'", "a.is_synthetic = FALSE", "a.is_off_topic = FALSE"]
    params: Dict[str, Any] = {}
    ...
    try:
        country_rows = db.execute_query("""
            ...
            FROM countries c LEFT JOIN (
                ...
                FROM articles WHERE is_synthetic = FALSE AND is_off_topic = FALSE GROUP BY country_code
        ...
        stats = db.execute_query("""
            ...
            FROM articles
            WHERE is_synthetic = FALSE AND is_off_topic = FALSE
        """)
